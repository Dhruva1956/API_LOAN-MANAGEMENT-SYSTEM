...
import utils, db, constants
from utils import generateExactMatchPattern


def loanRequest(args):
    #to find id
    ...
    #to find corresponding loan offer
...
def approveLoan(args):
    ...
    #to find corresponding offer of loan
    expr = {constants.loanInv.ID.name: data[constants.loanCust.LOAN_INVT_ID.name]}
    req_coll_2 = db.find_docs(constants.collectionName.loan_inventory.name, expr)
    if req_coll_2.count() == 0:
        return False
...
def listOtherUsers(role, username):
    notcontainingUsername = "^(?!" + username + "$)" 
...
*agents can see all admins, other agents and customers under them
*customers can view none
'''
